...
case "${2:-}" in
  */arm-none-eabi-*)
    if grep -q 'exec grun ' "$2"; then
      printf 'Bourne-Again shell script, ASCII text executable\n'
    else
      printf 'ELF 64-bit LSB pie executable, ARM aarch64\n'
    fi
    ;;
  *) exec /usr/bin/file "$@" ;;
esac
...
[[ -x "$HOME_DIR/.local/opt/k4a-arm-toolchain/bin/arm-none-eabi-gcc.k4a-real" ]]
mv "$HOME_DIR/.local/opt/k4a-arm-toolchain" "$HOME_DIR/.cache/k4a-toolchain.resumable"
# Model wrappers generated by the release that passed -f to grun.  The updater
# must repair them without modifying or re-extracting their real executables.
for tool in gcc as ld objcopy objdump strip; do
...
[[ -x "$HOME_DIR/.local/opt/k4a-arm-toolchain/bin/arm-none-eabi-gcc.k4a-real" ]]
grep -q 'exec grun ' "$HOME_DIR/.local/opt/k4a-arm-toolchain/bin/arm-none-eabi-gcc"
! grep -q 'grun -f' "$HOME_DIR/.local/opt/k4a-arm-toolchain/bin/arm-none-eabi-gcc"
[[ ! -d "$HOME_DIR/.cache/k4a-toolchain.resumable" ]]
mkdir -p "$HOME_DIR/.cache/k4a-toolchain.stale" "$HOME_DIR/.cache/k4a/toolchains"
touch "$HOME_DIR/.cache/k4a/toolchains/archive.tar.xz"
/bin/bash "$MANAGER" clean toolchain
[[ ! -e "$HOME_DIR/.local/opt/k4a-arm-toolchain" ]]
[[ ! -e "$HOME_DIR/.cache/k4a/toolchains" ]]
[[ ! -e "$HOME_DIR/.cache/k4a-toolchain.stale" ]]
mv "$FAKE_BIN/arm-none-eabi-gcc.pending" "$FAKE_BIN/arm-none-eabi-gcc"
rm -f -- "$FAKE_BIN/uname"
...
mkdir -p "$HOME_DIR/.local/opt/k4a-arm-toolchain/bin" "$HOME_DIR/.cache/k4a/toolchains"
printf '%s/glibc/lib/ld-linux-aarch64.so.1\n' "$PREFIX" \
  >"$HOME_DIR/.local/opt/k4a-arm-toolchain/bin/arm-none-eabi-gcc"
touch "$HOME_DIR/.cache/k4a/toolchains/old-archive.tar.xz"
mv "$FAKE_BIN/arm-none-eabi-gcc" "$FAKE_BIN/arm-none-eabi-gcc.pending"
output=$(/bin/bash "$MANAGER" build btt-skr-mini-e3-v3)
grep -q 'modified the Arm toolchain in place' <<<"$output"
[[ ! -e "$HOME_DIR/.local/opt/k4a-arm-toolchain" ]]
[[ ! -e "$HOME_DIR/.cache/k4a/toolchains" ]]
