...
- Update both the Android app and Termux bridge. Current versions run the two
  ...
  data in the same nonblocking event-loop pass. UPDATE also applies the one
  Klipper compatibility patch: `CLOCK_MONOTONIC` replaces buggy legacy-kernel
...
- Inspect the `Stats` lines immediately before shutdown in `klippy.log`.
  ...
  stall.
- Impossible MCU-frequency swings with clean serial traffic indicate corrupt host
  timestamps; run UPDATE.
